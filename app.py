...
def generate_query(device_uuid, sensor_type, start_time, end_time):
    # Generate query
    query = 'select * from readings where device_uuid="' + device_uuid + '"'
...
def generate_summary_query(sensor_type, start_time, end_time):
    # Generate query
...
def request_device_readings(device_uuid):
    ...
    * type -> The type of sensor value a client is looking for
    ...
    if request.method == 'POST':
        ...
        # Generate query
        query = generate_query(device_uuid, sensor_type, start_time, end_time)
...
def request_device_readings_quartiles(device_uuid):
    ...
    * end -> The epoch end time for a sensor being created
    ...
    numpy_array = np.array(values)
    print(numpy_array)
    quartiles = np.quantile(numpy_array, [.25, .75]) # return first and third quartiles
...
def request_readings_summary():
    ...
    * end -> The epoch end time for a sensor being created
    ...
    # Generate query
    query = generate_summary_query(sensor_type, start_time, end_time)
